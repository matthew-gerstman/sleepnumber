...
import { SleepNumberFlexFrame } from "./platformAccessory";
 ...
 */
export class SleepNumberHomebridgePlatform implements DynamicPlatformPlugin {
  ...
  public readonly sleepNumberApi: SleepNumberAPI;

  constructor(
    public readonly log: Logger,
    public readonly config: SleepNumberConfig,
    ...
    if (!email || !password) {
      throw new Error(
        "Ignoring SleepNumber setup because email or password was not provided."
      );
    }

    this.sleepNumberApi = new SleepNumberAPI(email, password);
  ...
  authenticate = async () => {
    try {
      ...
      this.log.error(
        "Failed to authenticate with SleepIQ. Please double-check your username and password. Disabling SleepNumber plugin. Error:",
        err.error
      );
    }
  };
    ...
    const devices = [
      {
        ...
        constructor: SleepNumberFlexFrame,
      },
    ];
